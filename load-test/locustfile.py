...
class PerformanceTest(HttpUser):
    ...
    def air_quailty(self):
        response = self.client.get(f"{host_air}{BASE_URL}/air-sensors/{random.choice(air_sensors)}", name=f"/air-sensors/[sensor_id]")  
        assert response.ok, response.reason
        if not response.ok:
            print(response.text)
            response.raise_for_status()
        response = self.client.get(f"{host_air}{BASE_URL}/air-quality/loc/MENTENG", name=f"/air-quality/loc/[location]")
        if not response.ok:
            print(response.text)
            response.raise_for_status()  
    ...
    def scenario(self):
        ...
        # Register Merchant
        merchant = random.choices(string.ascii_lowercase, k=8) + random.choices(string.digits, k=2)
        ...
        response = self.client.post(f"{host_voucher}{url_auth}/register", json=data)
        assert response.ok, response.reason
        if not response.ok:
            print(response.text)
            response.raise_for_status()
        ...
        # Create Voucher
        data = {
            "name": f"The {random.choices(string.ascii_lowercase, k=5)}",
        ...
        response = self.client.post(f"{host_voucher}{url_voucher}", json=data, headers={"authorization": "Bearer " + token})
        if not response.ok:
            print(response.text)
            response.raise_for_status()
        ...
        response = self.client.post(f"{host_voucher}{url_voucher}/detail", json=data, headers={"authorization": "Bearer " + token})
        if not response.ok:
            print(response.text)
            response.raise_for_status()
        ...
        # Create User
        username = random.choices(string.ascii_lowercase, k=8) + random.choices(string.digits, k=2)
        ...
        response = self.client.post(f"{host_user}{url_user}", json=data)
        if not response.ok:
            print(response.text)
            response.raise_for_status()
        ...
        for i in range(6):
            ...
            response = self.client.get(f"{host_trip}{url_qr_generators}/{qrgenerator_id}/generate-qr", name=f"{url_qr_generators}/[qrgenerator_id]/generate-qr")
            if not response.ok:
                print(response.text)
                response.raise_for_status()   
            ...
            # Scan QR
            response = self.client.get(f"{host_trip}{url_qr_generators}/decode-qr?data={qrcode_image}", name=f"{url_qr_generators}/decode-qr")
            if not response.ok:
            ...
            response = self.client.post(f"{host_trip}{url_trip}", json=data)
            if not response.ok:
                print("trips", data)
                print(response.text)
                response.raise_for_status()
        ...
        response = self.client.post(f"{host_voucher}{url_redeem}", json=data)
        if not response.ok:
            print("redeem", data)
            print(response.text)
            response.raise_for_status()
        ...
        response = self.client.post(f"{host_voucher}{url_redeem}/{redeem_voucher_id}", json=data, name=f"{url_redeem}/[redeem_voucher_id]")
        if not response.ok:
