...
public class AirQualityService {
  ...
  public List<AirQualityProcessedSchema> getAqiByLocation(String location) {
    List<AirSensor> airSensors = airSensorRepository.findAirSensorsByLocation(location.toLowerCase());
    return airSensors.stream().map(AirSensor::getId).map(this::getAqi).collect(Collectors.toList());
  }
